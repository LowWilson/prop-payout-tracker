...
async function loadData(){
  const { data: firmData, error: firmErr } = await sb
    .from("prop_firms")
    .select("*")
    .order("created_at", { ascending: true });

  if(firmErr){
    alert("firm読み込みエラー: " + firmErr.message);
    return;
  }

  const { data: payoutData, error: payoutErr } = await sb
    .from("payouts")
    .select("*")
    .order("created_at", { ascending: false });

  if(payoutErr){
    alert("payout読み込みエラー: " + payoutErr.message);
    return;
  }
...
async function addPayout(){
  ...
  if(!firm || !amount || amount <= 0 || !currentUser){
    return;
  }
...
async function deleteFirm(id){
  if(!confirm("このProp firmを消す？関連payoutも消えるよ。")){
    return;
  }
